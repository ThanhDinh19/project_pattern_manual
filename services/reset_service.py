...
def hard_reset_all_data() -> None:
    ...
    try:
        # SQL Server: xóa theo thứ tự bảng con -> bảng cha
...
def _hard_reset_rows(
    ...
    try:
        ...
        removed_row_count = int((count_row or {}).get("row_count") or 0)
        ...
        if affected_import_ids:
            placeholders = ", ".join(["?"] * len(affected_import_ids))
            ...
            for import_id in affected_import_ids:
                ...
                total_rows_row = _fetchone_dict(cursor)
                total_rows = int((total_rows_row or {}).get("total_rows") or 0)
                ...
                sheet_count_row = _fetchone_dict(cursor)
                sheet_count = int((sheet_count_row or {}).get("sheet_count") or 0)
                ...
                image_count_row = _fetchone_dict(cursor)
                image_index_count = int((image_count_row or {}).get("image_index_count") or 0)
                ...
                if total_rows <= 0:
                    ...
                    cursor.execute(
                        ...
                        SET total_rows = ?, sheet_count = ?, image_index_count = ?
                        WHERE id = ?
                        """,
            ...
            if removed_import_ids:
                placeholders_removed = ", ".join(["?"] * len(removed_import_ids))
                cursor.execute(
                    f"""
...
def hard_reset_by_customer(customer: str) -> dict[str, int]:
    ...
    return _hard_reset_rows(
        row_where_sql="customer = ?",
        row_where_params=(customer,),
        joined_where_sql="pr.customer = ?",
...
def hard_reset_by_customer_and_season(customer: str, season: str) -> dict[str, int]:
    ...
    return _hard_reset_rows(
        row_where_sql="customer = ? AND season = ?",
        row_where_params=(customer, season),
        joined_where_sql="pr.customer = ? AND pr.season = ?",
